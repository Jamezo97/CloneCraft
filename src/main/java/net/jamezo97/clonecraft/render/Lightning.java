...
public class Lightning
{
		...
		for (int a = 0; a < points.length; a++)
		{
			...
			else
			{
				...
				if (branch && rand.nextInt(5) == 0)
				{
					...
					Lightning branch = new Lightning(points[a].x, points[a].y, points[a].z, points[a].x + (x2 - x1) / lineDistance
							* (rand.nextFloat() * 0.4f + 0.6f) * distance,
							points[a].y + (y2 - y1) / lineDistance * (rand.nextFloat() * 0.4f + 0.6f) * distance, points[a].z + (z2 - z1) / lineDistance
									* (rand.nextFloat() * 0.4f + 0.6f) * distance, noPoints, rand.nextInt(50) < 20, powered);
	...
	public void render(float partial, float beginPower)
	{
		...
		for (int a = 0; a < points.length - 1; a++)
		{
			...
			// GL11.glColor4f(0.1f*powerSqr+0.1f, 0.2f*powerSqr+0.8f,
			// 0.2f*powerSqr+0.8f, 0.2f*powerSqr+0.8f);
