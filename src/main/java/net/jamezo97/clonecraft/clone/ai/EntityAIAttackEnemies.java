	...
	public boolean shouldExecute()
	{
		if (clone.getOptions().fight.get())
		{
			if (clone.getAttackTarget() == null || (!clone.shouldProvokeAttack(clone.getAttackTarget()) && !clone.canAttackEntity(clone.getAttackTarget())))
			{
				...
				{
					// If the current target is because of an act of revenge,
					// and is not a selected enemy
	...
	public void updateTask()
	{
		if (clone.getAttackTarget() != null && clone.getAttackTarget().isEntityAlive())
		{
			if (clone.canAttackEntity(clone.getAttackTarget()))
			{
				...
				if (clone.prevPosX == clone.posX && clone.prevPosY == clone.posY && clone.prevPosZ == clone.posZ
						&& clone.getAttackTarget().getHealth() == lastHealth)
				{
					noImprovement++;
