...
public class EntityAIShare extends EntityAIBase
{
	...
	public boolean shouldExecute()
	{
		...
		/*
		 * TypeCheck[][] checks = new TypeCheck[][]{ InventoryClone.CHECK_WOOD,
		 * InventoryClone.CHECK_STONE, InventoryClone.CHECK_IRON,
		 * InventoryClone.CHECK_GOLD, InventoryClone.CHECK_DIAMOND}; for(int a =
		 * 0; a < checks.length; a++){ for(int b = 0; b < checks[a].length;
		 * b++){ typeChecks.add(checks[a][b]); } }
		 */
	...
	public void resetTask()
	{
		if (this.isStackAvailable(itemOnOffer) && toShareWith.isEntityAlive() && toShareWith.getDistanceSqToEntity(clone) < 16)
		{
			...
			// Important to copy it here, otherwise the passed stack is
			// modified.
