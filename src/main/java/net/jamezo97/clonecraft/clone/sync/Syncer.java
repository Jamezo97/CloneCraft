...
public class Syncer
{
	...
	public void tick()
	{
		...
		{
			if (prev == null || !prev.contains(current.get(a))
					|| (CloneCraft.INSTANCE.config.SYNC_FORCE != 0 && clone.ticksExisted % CloneCraft.INSTANCE.config.SYNC_FORCE == 0))
			{
		...
		if (newPlayers.size() > 0)
		{
			...
			for (int a = 0; a < syncs.length; a++)
			{
				if (syncs[a].getChannel() == 0/*
											 * || (!flag &&
											 * syncs[a].getChannel() == 1)
											 */)
				{
	...
	public static void init(ArrayList<Sync> syncs)
	{
		...
		syncs.add(new SyncBuildData(ID_BUILD));
		syncs.add(new SyncIgnoreItems(ID_IGNOREITEMS));
		syncs.add(new SyncBuildIndex(ID_BUILDINDEX));
		syncs.add(new SyncBuildSpeed(ID_BUILDSPEED));
	}
