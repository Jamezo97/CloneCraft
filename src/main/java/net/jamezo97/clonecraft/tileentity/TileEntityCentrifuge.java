...
//		}
        ...
        if(worldObj.isRemote){
        	if((cookTime>0 || speed > 0.1)){
        		...
        		if(tick % 1 == 0){
        			worldObj.spawnParticle("smoke", xCoord + ventPositionsX[this.blockMetadata], yCoord+0.625, zCoord + ventPositionsZ[this.blockMetadata], 0.0D, 0.2D, 0.0D);
        		}
        		if(tick % 2 == 0){
        			worldObj.spawnParticle("flame", xCoord + ventPositionsX[this.blockMetadata], yCoord+0.625, zCoord + ventPositionsZ[this.blockMetadata], 0.0D, 0.09D, 0.0D);
        		}
        	}else if(isBurning()){
        		if(tick % 4 == 0){
        			worldObj.spawnParticle("smoke", xCoord + ventPositionsX[this.blockMetadata], yCoord+0.625, zCoord + ventPositionsZ[this.blockMetadata], 0.0D, 0.05D, 0.0D);
        		}
        		if(tick % 5 == 0){
        			worldObj.spawnParticle("flame", xCoord + ventPositionsX[this.blockMetadata], yCoord+0.625, zCoord + ventPositionsZ[this.blockMetadata], 0.0D, 0.025D, 0.0D);
        		}
			}
		}else{
			checkSendItems();
