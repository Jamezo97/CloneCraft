...
public class Reflect
{
	...
	private static Field getFieldTypeAfterFieldType(Class<?> type1, int type1Index, Class<?> type2, int type2Index, Class<?> containingClass,
			String nameForError)
	{
			...
			if (Map.class.isAssignableFrom(f.getType()))
			{
				...
				try
				{
					...
					if (entry instanceof Entry)
					{
						...
						{
							if (keyType.isAssignableFrom(theEntry.getKey().getClass()) && valueType.isAssignableFrom(theEntry.getValue().getClass()))
							{
								return f;
							}
						}
