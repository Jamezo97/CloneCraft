...
public abstract class GuiScrollable extends Gui{
	...
	{
		this(xPosition, yPosition, width, height, new ResourceLocation("clonecraft:textures/gui/ScrollableTex2.png"), 9, 2, 7, 64);
	}
	...
	public void setDimensions(int width, int height,  ResourceLocation resource, int scrollBarWidth, int scrollBarBorder, int buttonHeight, int textureSize)
	{
...
//			System.out.println(this.height + ", " + this.getEntryHeight());
			...
			for(int a = beginEntryFloor; a < beginEntryFloor+maxEntryCount; a++)
			{
				...
				if(mY >= minY && mY < maxY)
				{
					this.entryClicked(a, mX - this.xPosition, mY - minY);
				}
...
//		System.out.println(this.height + ", " + this.getEntryHeight());
		...
		for(int a = beginEntryFloor-1; a < beginEntryFloor+maxEntryCount+1; a++)
		{
			...
			this.renderEntry(a, this.width - scrollBarWidth, this.getEntryHeight(), mX - minX, mY - minY);
	...
	public abstract void entryClicked(int entryIndex, int mouseX, int mouseY);
	
	public abstract void renderEntry(int entryIndex, int width, int height, int mX, int mY);
