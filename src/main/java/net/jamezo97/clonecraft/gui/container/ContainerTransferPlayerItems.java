...
public class ContainerTransferPlayerItems extends Container
{
	...
	public ContainerTransferPlayerItems(EntityPlayer mainPlayer, EntityClone other)
	{
		...
		for (int j = 0; j < 2; j++)
		{
			for (int k = 0; k < 2; k++)
			{
				addSlotToContainer(new SlotArmorOtherPlayer(this, inventoryplayer, inventoryplayer.getSizeInventory() - 1 - k - (j * 2), 8 + (18 * j),
						89 + k * 18, (j * 2) + k));
			}
		}
		for (int k = 0; k < 3; k++)
		...
		for (int j = 0; j < 2; j++)
		{
			for (int k = 0; k < 2; k++)
			{
				addSlotToContainer(new SlotArmorOtherPlayer(this, inventoryplayer1, inventoryplayer1.getSizeInventory() - 1 - k - (j * 2), 134 + (18 * j),
						89 + k * 18, (j * 2) + k));
			}
		}
		for (int k = 0; k < 3; k++)
