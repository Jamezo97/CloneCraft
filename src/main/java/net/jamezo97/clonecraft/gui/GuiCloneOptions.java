...
public class GuiCloneOptions extends GuiScreen
{
	...
	public void initGuiComponents()
	{
		...
		for (; a < optionsAmount - 1; a++)
		{
			...
			if (option != null)
			{
				buttonList.add(new ButtonCloneOption(-100, spacing + (spacing + bWidth) * (a / qDown), spacing + topSectionHeight_b + spacing
						+ (spacing + bHeight) * (a % qDown), bWidth, bHeight, option, scale));
			}
		}

		buttonList.add(new GuiColourButton(0, spacing + (spacing + bWidth) * (a / qDown), spacing + topSectionHeight_b + spacing + (spacing + bHeight)
