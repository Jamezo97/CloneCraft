...
public class ItemRotateMapper extends Item
{
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int posX, int posY, int posZ, int side, float f1, float f2, float f3)
	{
		if (world.isRemote)
		{
			...
			if (currentBlock == null || currentBlock != clicked)
			{
				...
				if (currentBlock != null && currentBlock != clicked)
				{
					player.addChatMessage(new ChatComponentText("Different block. Reset selection.").setChatStyle(new ChatStyle()
				...
				player.addChatMessage(new ChatComponentText("First block selected. (" + metas[index] + ")").setChatStyle(new ChatStyle()
						.setColor(EnumChatFormatting.GREEN)));

				player.addChatMessage(new ChatComponentText("Now select three of the same block in order of increasing rotation.").setChatStyle(new ChatStyle()
						.setColor(EnumChatFormatting.DARK_GREEN)));

				player.addChatMessage(new ChatComponentText("Rotation is clockwise when viewed from above.").setChatStyle(new ChatStyle()
			...
			else if (currentBlock == clicked)
			{
				...
				player.addChatMessage(new ChatComponentText("Set block " + (index + 1) + " (" + metas[index] + ")").setChatStyle(new ChatStyle()
						.setColor(EnumChatFormatting.GREEN)));
				...
				if (index == 3)
				{
					...
					{
						player.addChatMessage(new ChatComponentText("Mapped rotations!").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_PURPLE)));
	...
	public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player)
	{
		if (world.isRemote)
		{
			...
			player.addChatMessage(new ChatComponentText("Reset selections").setChatStyle(new ChatStyle().setColor(EnumChatFormatting.DARK_GREEN)));
		}
