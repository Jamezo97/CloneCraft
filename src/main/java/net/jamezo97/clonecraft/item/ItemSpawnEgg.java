...
public class ItemSpawnEgg extends Item{
	...
	public String getItemStackDisplayName(ItemStack par1ItemStack) {
		...
		return (par1ItemStack.getItemDamage()==1?"":StatCollector.translateToLocal("cc.unactivated") + " ") + "\247" + CloneCraftHelper.getClosestColourChar(dna.getColour(par1ItemStack)) + dna.getCurrentEntityNameTrans() + "\247f " + StatCollector.translateToLocal("cc.item.spawnegg.name");
	}
...
/*
	...
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10) {
        ...
        else
        {
            ...
            Entity entity = new ItemData(par1ItemStack).spawnEntity(par4 + 0.5, par5+addY, par6 + 0.5, par3World);//spawnCreature(par3World, par1ItemStack.getItemDamage(), (double)par4 + 0.5D, (double)par5 + d0, (double)par6 + 0.5D);
...
//				e.updateData();
	...
	public void getSubItems(Item item, CreativeTabs par2CreativeTabs, List list) {
		...
		for(Entry<Integer, Class> entry : idToClass.entrySet()){
			...
			if(((EntityLiving.class.isAssignableFrom(entityClass) || CloneCraftHelper.isValid(classToString, entityClass))) && !CloneCraftHelper.isInvalid(classToString, entityClass)){
				data.fill(id);
				egg = new ItemStack(item, 1, 1);
				data.save(egg);
				list.add(egg);
