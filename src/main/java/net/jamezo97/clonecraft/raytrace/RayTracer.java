	...
	public static Point rayTraceSimpleGetEnd(World world, Vec3 va, Vec3 vb, int accuracy, BlockChecker checker){
		...
		for(int a = 1; a <= accuracy; a++){
			...
			if(!checker.isBlockSeeThru(b, ix, iy, iz, world)){
				if(x-ix > b.getBlockBoundsMinX() && x-ix < b.getBlockBoundsMaxX() &&
						y-iy > b.getBlockBoundsMinY() && y-iy < b.getBlockBoundsMaxY() &&
						z-iz > b.getBlockBoundsMinZ() && z-iz < b.getBlockBoundsMaxZ()){
					return new Point(va.xCoord + (((float)a) / ((float)accuracy))*dx, 
							va.yCoord + (((float)a) / ((float)accuracy))*dy, 
							va.zCoord + (((float)a) / ((float)accuracy))*dz);
				}
