...
/**
 * Helps bridge the gap between server and client. Can be used to send/receive
 * schematic files between the client and server, by splitting them up into
 * smaller packets and then recompiling them back on the other side.
 * 
 * @author James
 *
 */
public class SchematicList
{
	...
	public void onUpdate()
	{
		...
		for (int a = 0; a < builders.size(); a++)
		{
			if (builders.get(a).shouldDiscard())
			{
				...
				{
					builder.sender.addChatMessage(new ChatComponentText("Full Schematic File was not received. Aborting build.").setChatStyle(new ChatStyle()
	 ...
	 * 
	 * @param schem
	 *            The schematic to send
	 * @param sendTo
	 *            Who to send it to. If null, then it is sent to the client.
	 * @param The
	 *            final Packet to send once the rest have been sent.
	 */
	...
	public void reloadSchematics()
	{
		...
		for (int a = 0; a < schematics.size(); a++)
		{
			for (int b = 0; b < schematicFiles.size(); b++)
			{
				...
				{
					// Remove the file, as we won't be using it later on because
					// we handle it here.
		...
		// Now we have an array of schematic files, which are new. So let's load
		// them!
	...
	public String getSchematicNameFromFile(File schematicFile)
	{
		...
		localizedName = localizedName.substring(this.baseFolder.getAbsolutePath().length() + 1);// Remove
																								// the
																								// "C:\\Users/AppData.../CloneCraft/Schematics/"
																								// part
		localizedName = localizedName.substring(0, localizedName.length() - 10);// Remove
																				// the
																				// .schematic
																				// at
																				// the
	...
	/**
	 * Deletes any Buffers stored on the GPU which are no longer needed. However
	 * if the schematic is rendered again the buffers will be reloaded.
	 */
